...
Page({
    ...
    scenicIntro: function (e) {
        ...
        if (this.data.goToUrl) {
            wx.navigateTo({
                url: '../pageHtml/pageHtml?id=' + this.data.goToUrl
            });
        }
    },
    /**
    ...
    */
    ...
    bookTicket: function () {
        ...
        var payMoney = this.data.isSpecial ? this.data.luAllMoney : this.data.allMoney;//得到总钱数
        ...
        if (payMoney == '' || payMoney == undefined) {
            wx.showModal({
                title: '提示',
                content: '请填写您预定的价格！',
            })
        ...
        if ((phoneNumb != "" && that.data.sixCodeDisplay == true) || (phoneNumb != "" && that.data.sixCode != "")) {
            ...
            //sessionChoose 1是带sessionID的GET方法  2是不带sessionID的GET方法, 3是带sessionID的Post方法,4是不带sessionID的Post方法
            util.HttpRequst(true, "ztc/product/createCustomizeOrder", 3, that.data.shanshanSessionId, llkParam, "POST", false, function (res) {
                ...
                if (res.code == 200) {
                    ...
                    wx.navigateTo({
                        url: '../payResult/payResult?orderId=' + res.orderId,
                    });
    ...
    chooseMoreDate: function (e) {
        ...
        wx.navigateTo({
            url: '../../common/theDay/theDay?productId=' + this.data.shanProductCode + '&orderLimitTime=' + this.data.gOrderTime + str,
        });
    },
    ...
    searchProductDetail: function (localPoint) {
        var that = this;
        //sessionChoose 1是带sessionID的GET方法  2是不带sessionID的GET方法, 3是带sessionID的Post方法,4是不带sessionID的Post方法
        util.HttpRequst(false, "ztc/product/productDetail", 1, wx.getStorageSync('sessionId'), {
            "productId": that.data.shanProductCode,
            "location": localPoint
        }, "GET", true, function (res) {
            // console.log('searchProductDetail', res);
            if (res.code == 200) {
                var repDetail = res.detail ? res.detail : '';
                var note_arr = [];
    ...
    getLocationProductData: function (location) {
        ...
        util.HttpRequst(false, "ztc/product/productDetail", 1, wx.getStorageSync('sessionId'), {
            ...
            // console.log('getLocationProductData', res)
            if (res.code == 200) {
                that.setData({
                    gUpList: res.upList ? res.upList : [], // 去程-上车点
                    gDownList: res.downList ? res.downList : [], // 返程-下车点
    ...
    */
    ...
    onLoad: function (options) {
        ...
        if (options.isSpecial != undefined && options.isSpecial == 'true') {
            wx.setNavigationBarTitle({ title: '特殊定制' });// 动态修改标题
            this.setData({ isSpecial: true });
        }
    ...
    getComments: function () {
        var that = this;
        //sessionChoose 1是带sessionID的GET方法  2是不带sessionID的GET方法, 3是带sessionID的Post方法,4是不带sessionID的Post方法
        util.HttpRequst(true, "ztc/product/commentList", 1, wx.getStorageSync("sessionId"), {
